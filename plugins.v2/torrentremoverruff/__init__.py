...
class TorrentRemoverRuff(_PluginBase):
    ...
    def _handle_action(
        self,
        downloader: str,
        downlader_obj: Qbittorrent | Transmission,
        torrents: set[TorrentInfo],
    ):
        action_map = {
            "pause": {
    ...
    def _execute_removal_strategy(
        self,
        decision_generator_func: Callable[[], Generator[tuple[TorrentInfo, bool, bool], bool, None]],
    ) -> set[TorrentInfo]:
        """
        通用的策略执行器，用于处理删种逻辑。

        生成器产出 (torrent_info, strategy_should_remove, strategy_should_break),
        并通过 .send(actually_removed) 反馈实际是否删除，供策略更新内部状态,
        策略只需关注自身的核心判断。
        ...
        try:
            item, strategy_should_remove, strategy_should_break = next(decisions)
            while True:
                should_remove = (
                    strategy_should_remove
                    and self.__matches_remove_condition(item)
                    and self.__matches_complateonly(item)
                )
                ...
                if should_remove:
                    ...
                    if self._samedata:
                        remove_keys.add(key)
                if strategy_should_break and not self._samedata:
                    break
                item, strategy_should_remove, strategy_should_break = decisions.send(should_remove)
    ...
    def _remove_by_condition(self, torrents: Iterable[TorrentType]) -> set[TorrentInfo]:
        """条件删种：对每个种子检查移除条件，始终考虑移除，永不提前终止"""

        def make_decisions():
            for torrent in torrents:
                torrent_info = self.__format_torrent_info(torrent)
                yield torrent_info, True, False
    ...
    def _remove_by_freespace(self, sorted_torrents: list[TorrentType]) -> set[TorrentInfo]:
        ...
        def make_decisions():
            ...
            for torrent in sorted_torrents:
                ...
                strategy_should_remove = need_space_bytes > 0
                strategy_should_break = need_space_bytes <= 0
                actually_removed = yield (
                    torrent_info,
                    strategy_should_remove,
                    strategy_should_break,
                )
                if actually_removed:
    ...
    def _remove_by_maximum_count(self, sorted_torrents: list[TorrentType]) -> set[TorrentInfo]:
        ...
        def make_decisions():
            ...
            for i, torrent in enumerate(sorted_torrents):
                torrent_info = self.__format_torrent_info(torrent)
                strategy_should_remove = i < remove_count
                strategy_should_break = i >= remove_count
                yield torrent_info, strategy_should_remove, strategy_should_break
    ...
    def _remove_by_maximum_size(self, sorted_torrents: list[TorrentType]) -> set[TorrentInfo]:
        ...
        def make_decisions():
            ...
            for torrent in sorted_torrents:
                ...
                torrent_info = self.__format_torrent_info(torrent)
                strategy_should_remove = need_remove_size_gb > 0
                strategy_should_break = need_remove_size_gb <= 0
                actually_removed = yield (
                    torrent_info,
                    strategy_should_remove,
                    strategy_should_break,
