...
ALLOWED_EVIDENCE_CODES = frozenset(item.value for item in AiAttributionEvidenceCode)
...
def _parse_enum(enum_type: type[Any]) -> Callable[[object], object]:
    """在 LLM 边界把合法文本值显式转换为稳定枚举。"""
...
class _AiAttributionSuggestion(_SafeModel):
    """AI adapter 私有的 LLM 单项响应模型。"""

    item_id: str = Field(pattern=r"^item_[0-9]{3}$")
    decision: Annotated[AiAttributionDecision, BeforeValidator(_parse_enum(AiAttributionDecision))]
    media_type: Annotated[MediaType, BeforeValidator(_parse_enum(MediaType))]
    ...
    confidence: Annotated[AiAttributionConfidence, BeforeValidator(_parse_enum(AiAttributionConfidence))]
...
class AiBatchResponse(_SafeModel):
    ...
    items: list[_AiAttributionSuggestion] = Field(max_length=MAX_ITEMS_PER_BATCH)
...
class AiAttributionAdapter:
    ...
    def _target_context(self, context: SubtitleTarget) -> AiTargetContext | None:
    ...
    def _host_context(evidence: FileAttributionEvidence) -> AiHostContext:
        ...
        try:
            safe_year = int(raw_year) if isinstance(raw_year, (str, int, float)) and raw_year != "" else None
        except (TypeError, ValueError):
            safe_year = None
    ...
    def _audit(
        ...
        *,
        ...
        suggestion: _AiAttributionSuggestion | None = None,
    ) -> AiAttributionAudit:
        ...
        return AiAttributionAudit(
            ...
            trigger_reason=AiAttributionTriggerReason(self._reason_key(trigger_reason)),
            outcome=outcome,
            reason_code=AiAttributionReasonCode(self._reason_key(reason_code)),
            media_type=MediaType(suggestion.media_type.value) if suggestion else MediaType.UNKNOWN,
            ...
            confidence=AiAttributionConfidence(suggestion.confidence.value) if suggestion else None,
            evidence_codes=[AiAttributionEvidenceCode(code) for code in evidence_codes],
        )

    @staticmethod
    def _validate_suggestion(
        suggestion: _AiAttributionSuggestion,
        *,
        target: AiTargetContext,
        ...
        if suggestion.decision.value != AiAttributionDecision.TARGET.value:
            return "decision_not_target"
        if suggestion.confidence.value != AiAttributionConfidence.HIGH.value:
            return "confidence_not_high"
        if suggestion.media_type.value != target.media_type.value:
    ...
    def _apply_suggestion(
        ...
        return evidence.model_copy(
            update={
                ...
                "media_type": MediaType(suggestion.media_type.value),
    ...
    async def attribute_files(
        ...
        *,
        ...
        while cursor < len(pending) and result.request_count < MAX_BATCHES_PER_CANDIDATE and not result.circuit_open:
            ...
            batch_suggestions: dict[str, _AiAttributionSuggestion] = {}
            batch_error = False
            ...
            try:
                ...
                for raw_item in raw_items:
                    ...
                    try:
                        suggestion = _AiAttributionSuggestion.model_validate(raw_item)
    ...
    async def attribute_requests(
        ...
        *,
        ...
        """把一般化文件归属请求接入内部 AI 接管实现。"""

        ai_inputs: list[AiAttributionInput] = []
        for index, request in enumerate(requests, start=1):
            local_key = f"file_{index:04d}"
            evidence = evidence_by_key.get(local_key)
            if evidence is None:
                continue
            ai_inputs.append(
                AiAttributionInput(
                    local_key=local_key,
                    logical_source_path=str(request.logical_source_path),
                    evidence=evidence,
                )
            )
        result = await self.attribute_files(
            context,
            candidate,
            snapshot,
            ai_inputs,
        ...
        return FileAttributionBatchResult(
            evidence_by_key=result.evidence_by_key,
            audits_by_key=result.audits_by_key,
